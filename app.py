...
class PressurePlotWidget(QtWidgets.QWidget):
    def __init__(self, line_names, colors, buffer_size=50000, parent=None, plots=None, x_y_label_offset=23):
...
class CycleMonitorTab(QtWidgets.QWidget):
    ...
    # Добавление новой записи
    def add_cycle(self, cycle_number: int, start_time: str, duration_sec: int, p_f_exp:float, p_f_b:float,  p_f_t_b:float,
                  gas_used: float, gas_output: float, recovery_naive:float,
                  recovery_with_collectors:float, recovery_max:float, gass_loss_on_collectors:float, total_input_product: float, cycle_on_mix: bool):
        self.table.insertRow(0)
...
class MainWindow(QtWidgets.QWidget):
    ...
    def add_cycle_monitor_data(self, data: dict):
        self.cycle_monitor_tab.add_cycle(cycle_number=data["number"],
                                         ...
                                         recovery_max=data["extraction_ratio_max"],
...
#        try:
    ...
    def process_rows(self, rows: List[Dict]):
        for row in rows:
            ...
            # except Exception as e:
            #     print("Ошибка при анализе цикла", e)
        ...
        for t, delayed_data in self.delay_buffer.pop_ready(current_timestamp_in_row):
            ...
            for i in range(4):
                    if adsorbers[i].get_last_stage_without_idle(t)[1] == "pressurization" and \
                        ...
                        self.adsorber_pressure_data_for_cyclogram.emit(f"Adsorber {i+1}", t, row[str(self.adsorbers[i].dpe_ppe_line)])
                        self.adsorbers[i].set_ppe_p(p3 = float(row['p3']), p4 = float(row['p4']))
                        ...
                        self.adsorber_pressure_data_for_cyclogram.emit(f"Adsorber {i+1}", t, row[str(self.adsorbers[i].dpe_ppe_line)])
                        self.adsorbers[i].set_dpe_p(p3 = float(row['p3']), p4 = float(row['p4']))
    ...
    try:
        with open(path, "r") as f:
            ...
            for line in lines:
                ...
                param_value = line.split("=")[1].split("#")[0].strip()
...
if __name__ == "__main__":
    ...
    adsorbers = init_adsorbers(dpe_ppe_line=experimental_params.get("dpe_ppe_line", 'p3'))
